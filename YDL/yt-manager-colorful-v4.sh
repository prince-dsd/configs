...
if [[ -z "${NO_COLOR:-}" ]]; then
    ...
    TITLE="${BOLD}${CYAN}"; SECTION="${BOLD}${MAGENTA}"; ACCENT="${BOLD}${BLUE}"
    SUCCESS="${BOLD}${GREEN}"; WARNING="${BOLD}${YELLOW}"; ERROR="${BOLD}${RED}"
    INFO="${BOLD}${CYAN}"; MUTED="${DIM}${WHITE}"; VALUE="${BOLD}${WHITE}"
else
    RESET=""; BLACK=""; RED=""; GREEN=""; YELLOW=""; BLUE=""; MAGENTA=""; CYAN=""; WHITE=""
    BOLD=""; DIM=""; UNDERLINE=""; BG_BLUE=""; BG_CYAN=""; BG_MAGENTA=""
    TITLE=""; SECTION=""; ACCENT=""; SUCCESS=""; WARNING=""; ERROR=""; INFO=""; MUTED=""; VALUE=""
fi
...
box_title() {
    ...
    printf "${SECTION}│${RESET} ${TITLE}%-58s${RESET} ${SECTION}│${RESET}\n" "$title"
    echo -e "${SECTION}╰────────────────────────────────────────────────────────────╯${RESET}"
}

menu_item() { printf "  ${BOLD}${CYAN}[%s]${RESET} %s\n" "$1" "$2"; }
status_ok() { echo -e "  ${SUCCESS}✔${RESET} $*"; }
status_warn() { echo -e "  ${WARNING}⚠${RESET} $*"; }
status_error() { echo -e "  ${ERROR}✖${RESET} $*"; }
status_info() { echo -e "  ${INFO}●${RESET} $*"; }
...
banner() {
    ...
    echo "╔══════════════════════════════════════════════════════════════════════════╗"
    echo "║                                                                          ║"
    echo "║                         Y T - D L P   M A N A G E R                    ║"
    echo "║                                                                          ║"
    echo "║                 ${MAGENTA}Interactive YouTube Downloader${CYAN}                  ║"
    echo "║                                                                          ║"
    echo "╚══════════════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
    echo -e "  ${MUTED}Directory:${RESET} ${VALUE}${DOWNLOAD_DIR}${RESET}"
    echo -e "  ${MUTED}Codec:${RESET}     ${VALUE}${VIDEO_CODEC_DESCRIPTION}${RESET}"
    echo -e "  ${MUTED}Container:${RESET} ${VALUE}${CONTAINER}${RESET}"
    hr
}
...
download_video() {
    ...
    if [[ $status -eq 0 ]]; then
        ...
        status_ok "Download completed successfully."
        ...
        status_warn "Download completed with errors."
...
url_session() {
    ...
    while true; do
        ...
        echo
        status_info "Fetching video information..."
...
main_menu() {
    ...
    while true; do
        ...
        menu_item "1" "Enter YouTube URL"
        menu_item "2" "Settings"
        menu_item "3" "Show versions"
        menu_item "4" "Open download directory"
